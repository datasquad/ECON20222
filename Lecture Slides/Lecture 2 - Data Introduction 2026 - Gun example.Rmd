...
output: 
  beamer_presentation:
    includes:
      in_header: C:/Rcode/RforQM/latex_complete.tex
...
Formulate a null hypothesis: ***the difference in population means ($\mu$ or `mu`) in `vcrime.pc` is equal to 0***. We use the `t.test` function. We deliver the `vcrime.pc` series for both years to `t.test`.
...
```
\normalsize
The p-value is 0.7117 and hence \textcolor{student}{it is very likely that we would get a difference like the one we see or bigger if the null hypothesis was true.}
...
Once $\widehat{Cov}(y,x)$ and $\widehat{Var}(x)$ are replaced by sample estimates we get \textcolor{student}{~ONE~} value which is drawn from a \textcolor{student}{random distribution.}
...
Much of empirical econometric analysis is about making the \textcolor{green}{exogeneity} assumption ($Corr(x,u)=0$) more plausible/as plausible as possible. But this begins with thinking why an explanatory variable $x$ could be \textcolor{red}{endogenous}.
