---
title: "Quantitative Methods - Computer Lab 2"
author: "Ralf Becker"
date: ""
output:
  html_document:
...
We continue to use the same data as in Comp_Lab1. So you may want to continue working in the same script file you created last week and add commands to that file. The last thing we did in the previous lab is that we turned the `CHAIN` and `STORE` variables into factor variables, `CHAINf` and `STOREf`. We will use these variables in some of the work following. So the minimum code you need from last week is the loading of the libraries, the import of the data and the creation of the two factor variables `CHAINf` and `STOREf`. But of course it is best to just continue in the script file you created for the previous computer lab.
...
* to become more comfortable with the way we work in R
...
Whenever you deal with real life data you should ensure that you understand the data characteristics. The easiest way to get a first impression of your data is to create some summary statistics of the variables you are using, here for instance for the `EMPFT` variable (number of full-time employees before the change in the minimum wage)
...
You will constantly have to deal with error messages and that is totally normal.
...
Once you see an error message, your inner Sherlok Holmes or [Song Ci](https://en.wikipedia.org/wiki/Song_Ci) needs to come out! Here there is sufficient information in the error message to perhaps see that the problem lies with the parenthesis! (Hint: you need as many opening as closing, and of the same type and the correct order). And once you fixed that you will find a second mistake.
...
Also google ("R table dnn"), use the help function (`?table`) or ask ChatGPT to figure out what the `dnn` optional input into the table function achieves.
...
![](Comp_Lab_2_2526_files/figure-html/unnamed-chunk-11-1.png)<!-- -->
...
![](Comp_Lab_2_2526_files/figure-html/unnamed-chunk-12-1.png)<!-- -->
...
![](Comp_Lab_2_2526_files/figure-html/unnamed-chunk-14-1.png)<!-- -->
...
![](Comp_Lab_2_2526_files/figure-html/unnamed-chunk-15-1.png)<!-- -->
...
![](Comp_Lab_2_2526_files/figure-html/unnamed-chunk-16-1.png)<!-- -->
...
![](Comp_Lab_2_2526_files/figure-html/unnamed-chunk-18-1.png)<!-- -->
...
If you do not remember what the role of `na.rm = TRUE` in the above code was, first re-run the code without that part. Do you get an error message or results which are not what you expect. Search the internet, check the help function (`?mean`) or ask an AI to figure out what this bit of the code does.
...
> Challenge task: If you are finished with the above before the end of the class (and only then), you could do the following. Plot a scatterplot with the December price of a small fries (`pfry_DEC`) against the size of the stores measured by full time employees in December (`EMPFT2`). They you can also estimate a regression model that relates `pfry_DEC` as the dependent variable with `EMPFT2` as the explanatory variable. 
