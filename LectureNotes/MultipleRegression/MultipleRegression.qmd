...
In a study relating college grade point average (GPA) to time spent in various activities. You distribute a survey to several students. Students are asked how many hours they spend each week in four activities: studying, sleeping, working, and leisure.  Any activity is put into one of the four categories, so that for each student: $study + sleep + work + leisure = 168$
Consider the following model, 

$$
GPA = \beta_0 + \beta_1 study + \beta_2 sleep + \beta_3 work + \beta_4 leisure + u.
$$
1. Holding sleep, work, and leisure fixed, increasing study by one additional hour will increase GPA by $\beta_1$ units.
`r torf(FALSE)`

2.Why can the computer not estimate this model using OLS as written?
```{r}
opts_ci <- sample(c(
  answer = "There is perfect multicollinearity among the regressors",
  "The sample size is too small",
  "One of the regressors has zero variance",
  "GPA is not measured correctly"
))
```
`r longmcq(opts_ci)`

3. If we solve for $leisure = 168 - study - sleep - work$ and substitute this into the model, which expression is correct?
```{r}
opts_ci <- sample(c(
  answer = "$GPA = (\\beta_0 + 168\\beta_4) + (\\beta_1 - \\beta_4)study + (\\beta_2 - \\beta_4)sleep + (\\beta_3 - \\beta_4)work + u$",
  "$GPA = \\beta_0 + \\beta_1 study + \\beta_2 sleep + \\beta_3 work + u$",
  "$GPA = 168 + \\beta_1 study + u$",
  "$GPA = (\\beta_0 - \\beta_4) + \\beta_1 study + \\beta_2 sleep + \\beta_3 work + u$"
))
```
`r longmcq(opts_ci)`
:::
...
y = \beta_0 + \beta_1 x_1 + \beta_2 x_2 + u,
$$
but $x_2$ is omitted from the regression. If $\beta_2 < 0$ and $Corr(x_1, x_2) < 0$, what is the bias in $\tilde{\beta}_1$?
