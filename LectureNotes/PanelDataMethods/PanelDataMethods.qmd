...
We could estimate @eq-woods by OLS, treating $a_i + u_{it}$ as a single **composite error**. This estimator is called **Pooled OLS** (POLS) as it uses all data pooled across both time periods, hence $2N$ observations. However, we have already assumed that $Cov(x_{it},a_i)\neq 0$ which means that Pooled OLS is biased (and inconsistent). This is often called **heterogeneity bias** which is just a different name for OVB in this circumstance.

**However**, with panel data, we can difference-out the unobserved fixed effect $a_i$. Let's write @eq-woods for both time periods $t=1$ and $t=2$.
...
Expressing @eq-mee in words, the change in $y_{it}$ between periods 1 and 2 is regressed on the change in the regressor(s) and a constant, using $N$ obs. Note that the variables are not defined for period 1 and so the dataset comprises just $N$ obs, whereas regressing @eq-woods could be estimated on $2N$ observations.
...
`r hide("Can you help me understand the answer")` The key to understand here is that the variable needs to have variation across all $i$ **after** differencing. Neither age nor gender have such variation. Everyones gender will remain unchanged and everyone will be one year older if observations are apart by a year. `r unhide()`
:::
...
Regression estimates constructed without differencing in the twins sample generate a schooling return of about 11%, remarkably similar to Mincer's estimate for the return to education. This can be seen in the first column of Table 6.2 (@fig-twins). The model that produces the estimates in column (1) includes age, age squared, a dummy for women,
and a dummy for whites. White twins earn less than black twins, an unusual result in the realm of earnings comparisons by race, though the gap here is not significantly different from zero.

The differenced @#eq-fds generates a schooling return of about 6%, a result shown in column (2) of Table 6.2. This is substantially below the short regression estimate in column (1). This decline may reflect ability bias in the short model. Yet, once again, more subtle forces may also be at work.
...
Including industry dummies into @eq-woods2 in order to estimate the effect of schooling on wages is a bad idea for the following reason.
...
#| echo: false
opts_p <- c(
   "industry dummies will be negatovely correlated with wages as industries with larger employment have lower wages",
   "industry dummies are actually unlikely to explain variation in wages as all industries employ people on a a wide range of salaries",
   "the industry dummy will influence the amount of schooling and therefore should not be included",
   answer = "the industry dummy is itself an outcome of schooling and therefore should not be included as an explanatory variable",
   "the industry dummy is unlikely to be correlated with schooling and therefore there is no need to include it"
)
...
Cunningham, S. (2021) [Causal Inference: The MixTape](https://mixtape.scunning.com/index.html), Chapter 8.
